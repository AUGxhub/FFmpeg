...
static const AVOption virtualbass_options[] = {
    ...
    { "strength", "set virtual bass strength", OFFSET(strength), AV_OPT_TYPE_DOUBLE, {.dbl=1}, 0, 10, FLAGS },
    {NULL}
};
...
static int config_input(AVFilterLink *inlink)
{
    ...
    s->cf = ff_get_audio_buffer(inlink, 2);
    if (!s->cf)
...
{
    return 2.5 * atan(0.9 * x) + 2.5 * sqrt(1. - SQR(0.9 * x)) - 2.5;
}
...
static void vb_stereo(AVFilterContext *ctx, AVFrame *out, AVFrame *in)
{
    ...
    double *cf = (double *)s->cf->extended_data[0];
